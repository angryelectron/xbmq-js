...
class XBee {
   ...
   * @param {callback} callback - (error, frame) Callback to handle errors and
   * unsolicieted incoming XBee frames.
   */
  constructor (serialport, xbeeAPI, callback) {
 ...
 * @param {callback} config.callback - (error, frame) Callback to handle errors and
 * unsolicieted incoming XBee frames.
 * @return {Promise} Resolves with {@link XBee}, rejects with Error.
 */
  static create (config) {
    if (!config || !config.apiMode || !config.port || !config.baud || !config.callback) {
    ...
    } else if (!config.callback || config.callback.length !== 2) {
    ...
    return new Promise((resolve, reject) => {
      ...
      serialport.on('open', () => {
        resolve(new XBee(serialport, xbeeAPI, config.callback))
      })
    })
  }
