...
class Xbmq {
  ...
  static create (xbeeConfig, mqttConfig, logger) {
    ...
    return XBee.create(xbeeConfig).then((instance) => {
      ...
      const mqtt = Mqtt.create(mqttConfig, mqttConfig)
   ...
   * Safe to call even if Xbee and Mqtt are not connected.
 ...
 */
  onMqttEvent (error, topic, message) {
    ...
    try {
      const frame = this.convertToFrame(message)
 ...
 */
  convertToFrame (message) {
    const frame = JSON.parse(message) // will throw if message is not stringified JSON
