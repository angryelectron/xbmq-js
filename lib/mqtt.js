 ...
 * @param {callback} config.callback - (error, topic, message) Called on errors
 * or unsolicited incoming mqtt messages.
 */
  static create (config) {
    if (!(config.broker && config.topic && config.callback)) {
    ...
    let client = MqttClient.connect(config.broker, mqttOptions)
